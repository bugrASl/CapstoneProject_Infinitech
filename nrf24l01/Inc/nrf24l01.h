 ...
 *
 *   BSAU (STM32L432KC) — Transmitter:  #define NRF_ROLE_TX before including
 *   CPCU (STM32H755ZI) — Receiver:     #define NRF_ROLE_RX before including
 *
 * Common code (SPI access, pin control, register map, init sequence) is
 * always compiled. TX-only and RX-only functions are conditionally compiled
 * to minimize flash usage on the resource-constrained devices.
 */
...
#define NRF_SPI_TIMEOUT_MS      15              /* SPI per-transaction timeout (was 10, loosened) */
/*==========================================================================================================*/
...
#define NRF_REG_RX_ADDR_P0      0x0A            /* Receive address data pipe 0 (5 bytes) */
#define NRF_REG_RX_ADDR_P1      0x0B            /* Receive address data pipe 1 (5 bytes) */
...
/*============== FIFO Status Register Bit Masks ============================================================*/
...
#define NRF_CONFIG_CRCO         (1 << 2)        /* 0=1byte CRC, 1=2byte */
...
#define NRF_RF_SETUP_RF_DR_HIGH (1 << 3)        /* 0=1Mbps, 1=2Mbps */
#define NRF_RF_SETUP_RF_DR_LOW  (1 << 5)        /* 1=250kbps (with DR_HIGH=0) */
...
typedef enum {
    ...
    NRF_ERR_NOT_DETECTED,                       /* Chip not responding (register readback mismatch) */
    NRF_ERR_RX_EMPTY,                           /* No payload available */
    NRF_ERR_TX_FIFO_FULL,
    NRF_ERR_TX_MAX_RT,                          /* Max retransmits reached (packet lost) */
    NRF_ERR_TX_TIMEOUT,
} NRF_Status;
/*----------------------------------------------------------------------------------------------------------*/

/*-------------- Driver Handle Struct-----------------------------------------------------------------------*/
typedef struct {
