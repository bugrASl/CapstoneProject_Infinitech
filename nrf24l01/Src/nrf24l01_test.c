 ...
 * @author  bugrASl
 *
 * Each test is designed to be non-destructive: the radio state is
 * restored after each test so the chip remains operational.
 *
 * All tests use the LOG macro for output. The board prefix ([BSAU] or
 * [CPCU]) is inserted automatically by log.h based on LOG_BOARD_xxx.
 *
 * Rev 6.0: Added NRF_Test_PowerCycle(), guarded TestResult.
 */
...
/*
 * @brief   Lightweight, logs inline, returns bool.
 *
 * Defined locally so nrf24l01_test.c is self-contained.
 */
 ...
 *
 *   values: 01=3 bytes, 10=4 bytes, 11=5 bytes.
 */
TestResult NRF_Test_SPI(NRF_Handle *hnrf)
{
    ...
    pass                   &=   nrf_assert(readback == test_val, "SPI_Loopback",
                                        "wrote=0x%02X, read=0x%02X", test_val, readback);
    ...
    pass                   &=   nrf_assert(restored == saved, "SPI_Restore",
                                        "restored=0x%02X, expected=0x%02X", restored, saved);
...
/*
 * Strategy:
 *      Reads back every register NRF_Init() programmed and compares
...
TestResult NRF_Test_Registers(NRF_Handle *hnrf)
{
    ...
    /* SETUP_AW: (NRF_ADDR_WIDTH - 2) = 0x03 for 5-byte. Bits [1:0]. */
    ...
    /* RF_SETUP: RF_DR_LOW=1, RF_DR_HIGH=0, RF_PWR=0x03 → 250kbps 0dBm */
    pass                   &=   check_reg(hnrf, NRF_REG_RF_SETUP,
                                           NRF_RF_SETUP_RF_DR_LOW | NRF_RF_SETUP_RF_PWR_3,
                                            0x2E, "RF_SETUP");
 
    /* EN_AA: auto-ack pipe 0 only */
    pass                   &=   check_reg(hnrf, NRF_REG_EN_AA, 0x01, 0x3F, "EN_AA");
 
    /* EN_RXADDR: pipe 0 enabled */
    pass                   &=   check_reg(hnrf, NRF_REG_EN_RXADDR, 0x01, 0x3F, "EN_RXADDR");
 
    /* RX_PW_P0: payload width */
    pass                   &=   check_reg(hnrf, NRF_REG_RX_PW_P0,
                                           NRF_PAYLOAD_SIZE, 0x3F, "RX_PW_P0");
 
    /* SETUP_RETR: ARD=1500us (0x5), ARC=15 (0xF) → 0x5F */
    pass                   &=   check_reg(hnrf, NRF_REG_SETUP_RETR, 0x5F, 0xFF, "SETUP_RETR");
 
    /* CONFIG: role-dependent */
    uint8_t expected_config = NRF_CONFIG_EN_CRC | NRF_CONFIG_CRCO | NRF_CONFIG_PWR_UP;
#if defined(NRF_ROLE_RX)
    expected_config        |= NRF_CONFIG_PRIM_RX | NRF_CONFIG_MASK_TX_DS | NRF_CONFIG_MASK_MAX_RT;
#else
    expected_config        |= NRF_CONFIG_MASK_RX_DR;
#endif
    pass                   &= check_reg(hnrf, NRF_REG_CONFIG, expected_config, 0x7F, "CONFIG");
 ...
 *      flush, verify empty again.
 */
TestResult NRF_Test_FIFO(NRF_Handle *hnrf)
{
    LOG("NRF", "Test_FIFO", "RUN", "Verify FIFOs Status write/flush");
    ...
    /* Verify TX-FIFO is not Empty & Assert */
    fifo                    =   NRF_ReadReg(hnrf, NRF_REG_FIFO_STATUS);
    pass                   &=   nrf_assert((fifo & NRF_FIFO_TX_EMPTY) == 0,
                                            "FIFO_NonEmpty", "TX_EMPTY=%u (Post-Write expect 0)",
                                            (fifo >> 4) & 1);

    /* Flush FIFO */
    NRF_FlushTX(hnrf);

    /* Verify FIFOs are Empty Post-Flush & Assert*/
    fifo                    =   NRF_ReadReg(hnrf, NRF_REG_FIFO_STATUS);

    pass                   &=   nrf_assert((fifo & NRF_FIFO_TX_EMPTY) != 0,
                                            "FIFO_Flushed", "TX_EMPTY=%u after flush (expect 1)",
                                            (fifo >> 4) & 1);
 ...
 *
 * This tests that the power control path works end-to-end: CONFIG
 * register write, oscillator startup timing, and CE restoration.
 * Non-destructive — restores original state on exit.
 */
TestResult NRF_Test_PowerCycle(NRF_Handle *hnrf)
{
    ...
    pass                   &=   nrf_assert((cfg_down & NRF_CONFIG_PWR_UP) == 0, "PwrDown",
                                            "PWR_UP=%u after PowerDown (expect 0)",
                                            (cfg_down >> 1) & 1);

    /* Verify a register is still accessible while powered down (SPI works in power-down) */
    uint8_t ch_check        =   NRF_ReadReg(hnrf, NRF_REG_RF_CH);
    pass                   &=   nrf_assert(ch_check == hnrf->channel, "PwrDown_SPI",
                                            "RF_CH=0x%02X while powered down (expect 0x%02X)",
                                            ch_check, hnrf->channel);

    /* Power up */
    NRF_PowerUp(hnrf);     /* Includes 2 ms oscillator startup delay + CE restore for RX */
    uint8_t cfg_up          =   NRF_ReadReg(hnrf, NRF_REG_CONFIG);
    pass                   &=   nrf_assert((cfg_up & NRF_CONFIG_PWR_UP) != 0, "PwrUp",
                                            "PWR_UP=%u after PowerUp (expect 1)",
                                            (cfg_up >> 1) & 1);

    /* Verify CONFIG matches original (role bits, CRC, IRQ masks all preserved) */
    pass                   &=   nrf_assert(cfg_up == saved_config, "PwrRestore",
                                            "CONFIG=0x%02X after cycle (expect 0x%02X)",
                                            cfg_up, saved_config);
 ...
 *      Poll STATUS for the TX_DS or MAX_RT flag.
 *
 *      ┌──────────────────────────────────────────────────────────┐
 ...
 *      │ neither    │ timeout (75 ms)       │ FAIL (HW fault)     │
 *      └──────────────────────────────────────────────────────────┘
 *
 *      Why MAX_RT = PASS:
 *          The chip performed the full PTX flowchart from the datasheet:
 *          loaded payload → PLL lock → TX →
 *          RX for ACK → timeout → retransmit × 15 → set MAX_RT.
 *          Every block in the RF chain ran. The only thing missing is
 *          the other radio on the air.
 */
#if defined(NRF_ROLE_TX)
...
TestResult NRF_Test_TX(NRF_Handle *hnrf)
{
    ...
    if (result == NRF_OK)
    {
        pass               &=   nrf_assert(true, "TX_Result",
                                            "TX_DS=1 — ACK received (PLOS=%u ARC=%u)",
                                            lost, retx);
    }
    else if (result == NRF_ERR_TX_MAX_RT)
    {
        pass               &=   nrf_assert(true, "TX_Result",
                                            "MAX_RT=1 — no receiver, state machine OK (PLOS=%u ARC=%u)",
                                            lost, retx);
    }
    else if (result == NRF_ERR_TX_TIMEOUT)
    {
        pass               &=   nrf_assert(false, "TX_Result",
                                            "TIMEOUT — neither TX_DS nor MAX_RT in 75ms. STATUS=0x%02X",
                                            status);
    }
    else
    {
    ...
    LOG("NRF", "TX_Diag", "INFO", "STATUS=0x%02X (RX_DR=%u TX_DS=%u MAX_RT=%u) OBSERVE=PLOS:%u ARC:%u",
         status, (status >> 6) & 1, (status >> 5) & 1, (status >> 4) & 1, lost, retx);
...
TestResult NRF_Test_RX(NRF_Handle *hnrf)
{
    ...
    pass                   &=   nrf_assert((config & NRF_CONFIG_PWR_UP) != 0,
                                            "RX_PwrUp", "PWR_UP=%u (expect 1)",
                                            (config >> 1) & 1);
    pass                   &=   nrf_assert((config & NRF_CONFIG_PRIM_RX) != 0,
                                            "RX_PrimRx", "PRIM_RX=%u (expect 1)",
                                            config & 1);
    ...
    LOG("NRF", "Test_RX", "INFO", "Waiting up to %u ms for packet from BSAU...", NRF_TEST_RX_TIMEOUT_MS);
    ...
    if (!pkt_received)
    {
        pass               &=   nrf_assert(false, "RX_Packet",
                                            "no RX_DR after %u ms — is BSAU transmitting? STATUS=0x%02X",
    ...
    pass                   &=   nrf_assert(read_result == NRF_OK, "RX_ReadPayload",
                                            "NRF_ReadPayload returned %d (expect 0=OK)",
                                            read_result);
    ...
    LOG("NRF", "RX_PostRead", "INFO", "STATUS=0x%02X FIFO=0x%02X (RX_EMPTY=%u) payload[0..3]=%02X %02X %02X %02X",
         post_status, fifo, fifo & 0x01, payload[0], payload[1], payload[2], payload[3]);
    ...
    LOG("NRF", "RX_Summary", "INFO", "received 1+%lu packets in %lu ms", (unsigned long)extra_count, (unsigned long)elapsed);
...
TestResult NRF_Test_All(NRF_Handle *hnrf, const uint8_t addr[NRF_ADDR_WIDTH])
{
    ...
    LOG("NRF", "Test_All", all_pass ? "PASS" : "FAIL", "=== NRF SELF-TEST: %lu PASS, %lu FAIL ===",
